...
class SymbolTable {
    ...
    boolean addFunction(Sym functionSymbol) {
        if (
            functionScopes.containsKey(functionSymbol.name) && (
                ...
                !(((FunctionDec)lookupFunction(functionSymbol.name).def).params.equals(((FunctionDec)functionSymbol.def).params)) // making sure the parameters are the same as the prototype
            ) 
        ) {
            return false;
        }
